...
bluetooth.onUartDataReceived(serial.delimiters(Delimiters.Dollar), function () {
    ...
    if (UARTString.includes("p0") || UARTString.includes("p1") || UARTString.includes("p2")) {
        ...
        newData = UARTString.split(":")
        newUARTString = newData.pop()
        uART2Servo(newUARTString)
    } else if (UARTString.includes("speed")) {
        newData = UARTString.split(":")
        newUARTString = newData.pop()
        uART2ContinuousServo(newUARTString)
    } else if (UARTString.includes("wash")) {
        newData = UARTString.split(":")
        newUARTString = newData.pop()
        uART2ContinuousServo2(newUARTString)
    } else {
...
// send values from 0-180
function uART2ContinuousServo2 (stringData: string) {
